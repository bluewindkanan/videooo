...
| PC-002 | AI Director Pipeline | 让编导/审阅输出可被流水线消费，开始产生“可审查的中间产物” | `docs/00-project/vision.md#Direction（方向，不是排期）` | 核心能力落地 | 任务能生成脚本/分镜等结构化产物 + findings，并能导出成片初稿 | 直接暴露 LLM 质量、素材匹配与审阅机制的真实问题 | P1 | recommended | 当前主线：从“骨架”走向“真实可用闭环” | LLM 成本/延迟与输出不稳定；素材链接可用性不确定 |
| PC-003 | 素材两路径与失败分流 | 链接不可解析/不可下载时仍可完成生产（上传替代） | `docs/00-project/vision.md#Open-Assumptions（开放假设）` | 稳定性增强 | 用户在素材失败时得到明确原因，并可切换到上传素材继续推进 | 把“素材不可用”从偶发 bug 变成可管理分支 | P1 | candidate | 素材可用性是首要外部不确定性，必须工程化降级 | 上传与存储会引入容量/清理/权限与安全风险 |
...
- candidate_id: PC-002
- rationale: 目前已具备任务骨架（PC-001/shipped），下一步必须让“编导 + 审阅 + 产物结构化 + 合成初稿”成为真实可用的端到端路径，才能开始进入有效学习循环。
- success_definition: 至少产出“脚本/分镜（结构化）+ findings（结构化）+ 导出视频初稿”三类关键产物，并能在失败时留痕与复盘。

## Dependency / Sequencing Notes

- PC-001 是其他切片的前置：没有步骤状态与产物体系，就无法做可审查与可重试。
- PC-002 完成后，才能判断 PC-004（质量门槛）与 PC-006（回归资产）的最小有效形态；否则容易在“空目标”上做评测或 gate。
- PC-003 与 PC-002 强相关：素材获取失败会直接影响端到端可用性，但可根据实际素材失败频率决定优先级。
